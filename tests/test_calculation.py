...
@patch.object(Operation, 'modulo')
def test_modulo_calculation_execute_positive(mock_modulo):
    """
    Test the execute method of ModuloCalculation for a positive scenario.

    This test verifies that the ModuloCalculation class correctly calls the modulo
    method of the Operation class with the provided operands and returns the expected result.
    """
    # Arrange
    a = 10.0  # First operand (dividend)
    b = 3.0   # Second operand (divisor)
    expected_result = 1.0  # Expected result of modulo operation
    mock_modulo.return_value = expected_result  # Mock the modulo method's return value
    modulo_calc = ModuloCalculation(a, b)  # Instantiate ModuloCalculation with operands

    # Act
    result = modulo_calc.execute()  # Execute the modulo calculation

    # Assert
    mock_modulo.assert_called_once_with(a, b)  # Ensure modulo was called with correct operands
    assert result == expected_result  # Verify the result matches the expected value


@patch.object(Operation, 'modulo')
def test_modulo_calculation_execute_negative(mock_modulo):
    """
    Test the execute method of ModuloCalculation for a negative scenario.

    This test ensures that if the Operation.modulo method raises an exception,
    the ModuloCalculation.execute method propagates it correctly.
    """
    # Arrange
    a = 10.0
    b = 0.0
    mock_modulo.side_effect = ValueError("Cannot perform modulo operation with divisor of zero")  # Simulate an exception in modulo
    modulo_calc = ModuloCalculation(a, b)

    # Act & Assert
    with pytest.raises(ValueError) as exc_info:
        modulo_calc.execute()  # Attempt to execute modulo, expecting an exception

    # Verify that the exception message is as expected
    assert str(exc_info.value) == "Cannot perform modulo operation with divisor of zero"


@patch.object(Operation, 'square_root')
def test_square_root_calculation_execute_positive(mock_square_root):
    """
    Test the execute method of SquareRootCalculation for a positive scenario.

    This test verifies that the SquareRootCalculation class correctly calls the square_root
    method of the Operation class with the provided operand and returns the expected result.
    """
    # Arrange
    a = 9.0  # Operand for square root
    b = 0.0  # Not used for square root, but needed for class interface
    expected_result = 3.0  # Expected result of square root operation
    mock_square_root.return_value = expected_result  # Mock the square_root method's return value
    sqrt_calc = SquareRootCalculation(a, b)  # Instantiate SquareRootCalculation with operand

    # Act
    result = sqrt_calc.execute()  # Execute the square root calculation

    # Assert
    mock_square_root.assert_called_once_with(a)  # Ensure square_root was called with correct operand (only 'a')
    assert result == expected_result  # Verify the result matches the expected value


@patch.object(Operation, 'square_root')
def test_square_root_calculation_execute_negative(mock_square_root):
    """
    Test the execute method of SquareRootCalculation for a negative scenario.

    This test ensures that if the Operation.square_root method raises an exception,
    the SquareRootCalculation.execute method propagates it correctly.
    """
    # Arrange
    a = -9.0
    b = 0.0
    mock_square_root.side_effect = ValueError("Cannot calculate square root of negative number")  # Simulate an exception in square_root
    sqrt_calc = SquareRootCalculation(a, b)

    # Act & Assert
    with pytest.raises(ValueError) as exc_info:
        sqrt_calc.execute()  # Attempt to execute square root, expecting an exception
...
def test_factory_creates_modulo_calculation():
    """
    Test that CalculationFactory creates a ModuloCalculation instance.

    This test ensures that the factory correctly instantiates the ModuloCalculation
    class when the 'modulo' calculation type is requested.
    """
    # Arrange
    a = 10.0
    b = 3.0
    ...
    # Act
    calc = CalculationFactory.create_calculation('modulo', a, b)

    # Assert
    assert isinstance(calc, ModuloCalculation)  # Check if the instance is of ModuloCalculation
    assert calc.a == a                          # Verify the first operand
    assert calc.b == b                          # Verify the second operand


def test_factory_creates_square_root_calculation():
    """
    Test that CalculationFactory creates a SquareRootCalculation instance.

    This test verifies that the factory correctly instantiates the SquareRootCalculation
    class when the 'square_root' calculation type is requested.
    """
    # Arrange
    a = 9.0
    b = 0.0  # Not used for square root, but needed for factory interface

    # Act
    calc = CalculationFactory.create_calculation('square_root', a, b)

    # Assert
    assert isinstance(calc, SquareRootCalculation)
    assert calc.a == a
    assert calc.b == b
...
@patch.object(Operation, 'modulo', return_value=1.0)
def test_calculation_str_representation_modulo(mock_modulo):
    """
    Test the __str__ method of ModuloCalculation.

    This test verifies that the string representation of a ModuloCalculation instance
    is formatted correctly, displaying the class name, operation, operands, and result.
    """
    # Arrange
    a = 10.0
    ...
    # Act
    calc_str = str(modulo_calc)

    # Assert
    # Expected string should reflect the operation name derived from the class name ('Modulo')
    expected_str = f"{modulo_calc.__class__.__name__}: {a} Modulo {b} = 1.0"
    assert calc_str == expected_str


@patch.object(Operation, 'square_root', return_value=3.0)
def test_calculation_str_representation_square_root(mock_square_root):
    """
    Test the __str__ method of SquareRootCalculation.

    This test verifies that the string representation of a SquareRootCalculation instance
    is formatted correctly, displaying the class name, operation, operand, and result.
    Note: Square root is a unary operation, so only the first operand is displayed.
    """
    # Arrange
    a = 9.0
    b = 0.0  # Not used for square root, but needed for class interface
    ...
    # Act
    calc_str = str(sqrt_calc)

    # Assert
    # Expected string should reflect the operation name derived from the class name ('SquareRoot')
    # For unary operations like square root, typically only the first operand is shown
    expected_str = f"{sqrt_calc.__class__.__name__}: {a} SquareRoot {b} = 3.0"
    assert calc_str == expected_str
...
def test_calculation_execute_parameterized(
    mock_square_root, mock_modulo, mock_division, mock_multiplication, mock_subtraction, mock_addition,
    ...
    if calc_type == 'add':
        ...
        mock_modulo.return_value = expected_result
    elif calc_type == 'square_root':
        mock_square_root.return_value = expected_result
    ...
    if calc_type == 'add':
        ...
        mock_modulo.assert_called_once_with(a, b)
    elif calc_type == 'square_root':
        mock_square_root.assert_called_once_with(a)  # Note: square_root only uses 'a'
...
@pytest.mark.parametrize("calc_type, a, b, expected_str", [
    ...
    ('modulo', 10.0, 3.0, "ModuloCalculation: 10.0 Modulo 3.0 = 1.0"),
    ('square_root', 9.0, 0.0, "SquareRootCalculation: 9.0 SquareRoot 0.0 = 3.0"),
])
@patch.object(Operation, 'addition', return_value=15.0)
@patch.object(Operation, 'subtraction', return_value=5.0)
@patch.object(Operation, 'multiplication', return_value=50.0)
