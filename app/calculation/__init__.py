...
class ModuloCalculation(Calculation):
    """
    ModuloCalculation represents a modulo operation.
    
    By encapsulating the modulo logic here, we achieve a clear separation of 
    concerns, making it easy to adjust the modulo logic without affecting other calculations.
    """

    def execute(self) -> float:
        # Calls the modulo method from the Operation module to perform the modulo operation.
        return Operation.modulo(self.a, self.b)
    
@CalculationFactory.register_calculation('square_root')
class SquareRootCalculation(Calculation):
    """
    SquareRootCalculation represents a square root operation.
    
    This operation only uses the first parameter (self.a) as square root is a unary operation.
    By encapsulating the square root logic here, we achieve a clear separation of 
    concerns, making it easy to adjust the square root logic without affecting other calculations.
    """

    def execute(self) -> float:
        # Calls the square_root method from the Operation module to perform the square root.
        return Operation.square_root(self.a)